...
def enrich_with_llm(text: str, row: dict) -> dict:
    ...
    # Arabic name prompt returns JSON — request structured output to guarantee valid JSON
    extra_kwargs: dict = {}
    if is_arabic_name:
    ...
    # Parse JSON response for Arabic names
    ...
    return {
        ...
        "normalised_form": result_text.upper() if not is_arabic_name else result_text,
        "processing_method": "LLM",
        "model_version": MODEL,
        "confidence": 0.85,
