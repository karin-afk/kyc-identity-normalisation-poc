...
| F.30 | Korean surname Ryu/Yoo/Lyu family | ❌ FAIL |
...
| F.25 | Chinese short two-character name | ❌ FAIL |
...
```json
{"original_text": "أحمد سمير نصر عبد الناصر", "field_type": "full_name", "language": "ar"}
```
...
| | Expected | Got | Status |
|---|---|---|---|
| **field_type** | `lei_code` | `id_no` | ⚠️ mismatch |
| **language** | `en` | `en` | ✅ match |
| **confidence** | — | `0.90` | — |
| **latency** | — | `1.27s` | — |

> ⚠️ **Classification mismatch on field_type.** GPT-4o-mini returned `id_no` but expected `lei_code`. The router will process the field as `id_no` which may select the wrong strategy.
...
```json
{"original_text": "529900T8BM49AURSDO55", "field_type": "id_no", "language": "en"}
```
...
```json
{"original_text": "Алексей Юрьевич Ковалёв", "field_type": "unstructured_text", "language": "en"}
```
...
| **normalised_form** | `ALEKSEI YURYEVICH KOVALEV` | `  ` | ❌ FAIL |
...
```json
{"original_text": "李伟", "field_type": "unstructured_text", "language": "en"}
```
...
```json
{"original_text": "류민석", "field_type": "unstructured_text", "language": "en"}
```
...
|---|---|---|---|
| **method** | `TRANSLITERATION` or `TRANSLITERATE` | `CHARACTER_MAP` | ❌ FAIL |
| **normalised_form** | `RYU MINSEOK` | `` | ❌ FAIL |
...
| | Expected | Got | Status |
|---|---|---|---|
| **field_type** | `alias` | `full_name` | ⚠️ mismatch |
| **language** | `en` | `en` | ✅ match |
| **confidence** | — | `0.95` | — |
| **latency** | — | `1.39s` | — |

> ⚠️ **Classification mismatch on field_type.** GPT-4o-mini returned `full_name` but expected `alias`. The router will process the field as `full_name` which may select the wrong strategy.
...
```json
{"original_text": "Pierre-Henri Lefèvre dit Le Vieux", "field_type": "unstructured_text", "language": "en"}
```
...
```json
{"original_text": "Zahlungsziel: 05.09.2026, Betrag: 12.500 EUR", "field_type": "unstructured_text", "language": "en"}
```
