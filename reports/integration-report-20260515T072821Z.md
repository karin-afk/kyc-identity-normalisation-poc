...
```json
{"original_text": "山田 太郎", "field_type": "unknown", "language": "unknown"}
```
...
| **normalised_form** | `YAMADA TARO` | `None` | ❌ FAIL |
...
|---|---|---|---|
| **field_type** | `person_name` | `unknown` | ⚠️ mismatch |
| **language** | `ko` | `unknown` | ⚠️ mismatch |
| **confidence** | — | `0.00` | — |
| **latency** | — | `0.65s` | — |

> ⚠️ **Classification mismatch on field_type.** Classifier returned `unknown` but expected `person_name`. The router will process the field as `unknown` which may select the wrong strategy.

> ⚠️ **Classification mismatch on language.** Classifier returned `unknown` but expected `ko`. This may affect strategy selection (e.g. character map handler chosen for wrong language).
...
```json
{"original_text": "류민석", "field_type": "unknown", "language": "unknown"}
```
...
|---|---|---|---|
| **method** | `TRANSLITERATION` or `TRANSLITERATE` | `UNRESOLVED` | ❌ FAIL |
| **normalised_form** | `RYU MINSEOK` | `None` | ❌ FAIL |
